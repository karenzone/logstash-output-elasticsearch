...
IMPORTANT: Time-based routing in this plugin follows *one* model at a time. Do not mix them:
+
--
* *Data streams* — use <<plugins-{type}s-{plugin}-data_stream,`data_stream`>> and related settings.
* *{ilm} rollover* — Logstash writes to a *rollover alias*; backing indices are named using
  {ref}/indices-rollover-index.html#_using_date_math_with_the_rollover_api[Elasticsearch rollover naming]
  (for example `{now/d}-000001` via <<plugins-{type}s-{plugin}-ilm_pattern,`ilm_pattern`>>), not by setting
  `index => "prefix-%{+yyyy.MM.dd}"` for each day.
* *Explicit `index` names* — including daily (or other) rotation with Logstash `%{+...}` syntax on
  <<plugins-{type}s-{plugin}-index,`index`>> — is a separate approach from rollover aliases.
--
+
Rollover-based {ilm} and Logstash `index` date patterns solve different problems; combining them as
one mental model leads to misconfiguration. If you rely on `index` with dynamic substitution, set
<<plugins-{type}s-{plugin}-ilm_enabled,`ilm_enabled`>> to `false` unless you understand how the
rollover alias overrides `index` (see <<plugins-{type}s-{plugin}-ilm,the {ilm} section>>).
...
This is *not* the same as using Logstash date formats on <<plugins-{type}s-{plugin}-index,`index`>>
(for example `index => "logs-%{+yyyy.MM.dd}"`). The latter resolves a *different index name per event*
from `@timestamp`; rollover expects a *stable write alias* and creates new backing indices when
rollover runs.
...
Here, *date math* means Elasticsearch rollover index naming (such as `{now/d}` in the pattern), not
Logstash field syntax like `%{+yyyy.MM.dd}` on <<plugins-{type}s-{plugin}-index,`index`>>.
Date math can be used when specifying an `ilm_pattern`; see
...
NOTE: When <<plugins-{type}s-{plugin}-ilm_enabled,{ilm}>> is enabled and a rollover alias is in use,
the effective write target is the rollover alias (`ilm_rollover_alias` takes precedence over `index`).
Daily or dynamic `index` patterns apply when you are *not* using that rollover path, or when
<<plugins-{type}s-{plugin}-ilm_enabled,`ilm_enabled`>> is `false`. See
<<plugins-{type}s-{plugin}-ilm,the {ilm} section>> and
<<plugins-{type}s-{plugin}-ilm_rollover_alias,`ilm_rollover_alias`>>.
